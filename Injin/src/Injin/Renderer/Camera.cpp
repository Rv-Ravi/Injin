...
engin::Camera::Camera()
	:m_camPos(0.f,0.f,1.f), m_camMaxVel(6.f),m_viewMat(1.f),m_projMat(1.f),
	m_viewProjMat(1.f), m_zNear(0.01f), m_zFar(10.f), m_width(16.f), m_height(9.f)
{
}
...
engin::OrthoCamera::OrthoCamera(const glm::vec3& position)
	:Camera(position)
{
	calcViewMat();
	calcProjMat();
...
engin::OrthoCamera::~OrthoCamera()
{
}

void engin::OrthoCamera::mvCam(const engin::WindowGL& window, float dtime)
{
	if (window.getKeyState(GLFW_KEY_UP) == 1)
		m_camPos.y += m_camMaxVel.y * dtime;
	else if (window.getKeyState(GLFW_KEY_DOWN) == 1)
		m_camPos.y -= m_camMaxVel.y * dtime;

	if (window.getKeyState(GLFW_KEY_RIGHT) == 1)
		m_camPos.x += m_camMaxVel.x * dtime;
...
glm::vec3 engin::OrthoCamera::getCamDir() {
	return glm::vec3(0.f, 0.f, -1.f);
}

void engin::OrthoCamera::imGuiWindows() {

	ImGui::Text("Position : %.2f, %.2f, %.2f", m_camPos.x, m_camPos.y, m_camPos.z);

	ImGui::Text("Camera X Direction : %.2f, %.2f, %.2f", 1.0f, 0.0f,0.0f);
	ImGui::Text("Camera Y Direction : %.2f, %.2f, %.2f", 0.0f, 1.0f, 0.0f);
	ImGui::Text("Camera Z Direction : %.2f, %.2f, %.2f", 0.0f, 0.0f, 1.0f);

	ImGui::DragFloat3("Camera Velocity", &m_camMaxVel.x, 1.f, 1.f, 60.f, "%.2f");
	ImGui::DragFloat("Camera Width", &m_width, 1.f, 4.0f, 100.f, "%.2f");
	ImGui::DragFloat("Camera Height", &m_height, 1.f, 3.f, 100.f, "%.2f");
	ImGui::DragFloat("Camera Near", &m_zNear, 1.f, 0.01f, 5.f, "%.2f");
	ImGui::DragFloat("Camera Far", &m_zFar, 1.f, 1.f, 100.f, "%.2f");

}

void engin::OrthoCamera::calcViewMat()
{
	glm::vec3 m_camZ(0.f,0.f,1.f);
	glm::vec3 m_camX(1.f, 0.f, 0.f);
	glm::vec3 m_camY(0.f, 1.f, 0.f);
...
void engin::OrthoCamera::calcProjMat()
{
	m_projMat = glm::ortho(-m_width,m_width,-m_height,m_height, m_zNear, m_zFar);
}

void engin::OrthoCamera::calcViewProjMat()
{
...
engin::PerspectiveCamera::PerspectiveCamera(const glm::vec3& position)
	...
	m_camX(1.f, 0.f, 0.f), m_camY(0.f, 1.f, 0.f), m_camZ(0.f, 0.f, 1.f), m_camDir(-m_camZ)
{
	calcViewMat();
	calcProjMat();
	calcViewProjMat();
