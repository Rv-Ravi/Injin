...
vec4 directionalLitCalc(in vec3 normal){
	...
	if(material.isTextured == false)
	{
		...
		vec3 diffFac = max(dot(-light.direction,normal),0.f) * material.diffuseColor;
		vec3 specFac = pow(max(dot(reflect(light.direction,normal),normalize(camPos - fIn.objPos)),0.f),material.shinniness) * material.SpecularColor;

		return vec4((material.ambientColor + diffFac + specFac) * light.color, 1.f);
	}
	else{
...
vec4 pointlLitCalc(in vec3 normal){
	...
	if(material.isTextured == false)
	{
		...
		return vec4((material.ambientColor + diffFac + specFac) * light.color * attenFac, 1.f);
	}
	else{
...
vec4 SpotLitCalc(in vec3 normal){
	...
	if(material.isTextured == false)
	{
		if(orgVal > light.outerAngle)
		{
			...
			return vec4((material.ambientColor + diffFac + specFac) * light.color * attenFac * IntFac, 1.f);
		}
		else{
			return vec4(material.ambientColor , 1.f) * vec4(0.1f);
		}
